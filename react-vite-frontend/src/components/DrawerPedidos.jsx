...
export default function DrawerPedidos(props) {
    ...
    useEffect(() => {
        if (props.order) {
            ...
            setForm({
                empresaId: 1,
                nome: "Aniversário do Jhonatan",
                descricao: "Aniversário temático - 20 pessoas",
                valor: 10,
    ...
    return (
        ...
        >
            <div className="flex h-full w-full justify-end">
                <div
                    className={`w-full md:w-125 h-screen bg-[#fcfaff] flex flex-col shadow-2xl transition-transform duration-500 transform ${props.open ? "translate-x-0" : "translate-x-full"}`}
                ...
                >
                    ...
                    <div className="bg-[#EDE0F0] h-24 border-b border-[#896D9533] flex items-center px-8 justify-between">
                        ...
                        <button
                            className="text-2xl border w-8 h-8 rounded-full flex pt-1 items-center justify-center pb-1 cursor-pointer border-[#896D9533] hover:bg-[#f1e6f3]"
                            onClick={props.onClose}
                        >
                            ×
                        </button>
                    ...
                    <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
                        <div className="flex flex-col text-[#3D2B4F]">
                            ...
                            <input
                                className="border border-[#e8d8f0] rounded-2xl h-12 px-4 outline-none focus:border-[#896D95] bg-white shadow-sm"
                        ...
                        <div className="flex gap-4 text-[#3D2B4F]">
                            <div className="flex-1">
                                <label
                                    className="font-medium mb-2 font-title text-[#3D2B4F] block "
                                >
                                    Status
                                </label>
                                <select
                                    className="border border-[#e8d8f0] rounded-2xl h-12 px-4 outline-none w-full bg-white shadow-sm"
                                    value={form.status}
                            ...
                            <div className="flex-1">
                                <label className="font-medium mb-2 font-title text-[#3D2B4F] block">Data de entrega</label>
                                <input
                                    className="border font-medium border-[#e8d8f0] rounded-2xl h-12 px-4 outline-none w-full bg-white shadow-sm"
                                    type="date"
                                    value={form.prazo}
